...
//                var sublen = len - (Math.max(50, 20 + gfx.textWidth(edge.target.name)) / 2);
                var sublen = len - (parseInt(document.getElementById('circlesize').value)/2) ;
                var thirdlen = len/3;
...
function getCircleSize(node) {
  var docValue = document.getElementById('circlesize').value;
  var size = 55;
  if(docValue.toUpperCase() == "AUTO") {
    size =  Math.max(55, 25 + gfx.textWidth(node.name));
  } else {
    try {
      var tmpVal = parseInt(docValue);
      size = tmpVal;
    } catch(error) {
      size = 55;
    }
